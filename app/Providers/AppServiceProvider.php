...
class AppServiceProvider extends ServiceProvider
{
    ...
    private function observerConfiguration()
    {
    ...
    private function _modifySidebarAndHeader()
    {
        TallStackUi::personalize('sideBar')
            ->block('desktop.wrapper.second', 'dark:bg-sidebar bg-[#282828] dark:border-border flex grow flex-col gap-y-5 overflow-y-auto border-r-[1px] border-border px-2 pb-4')
        ...
        TallStackUi::personalize('sideBar.item')
            ->block('item.state.current', 'text-white bg-[#ff6425]/50 dark:bg-dark-700 dark:text-white');
        TallStackUi::personalize('sideBar.item')
            ->block('item.state.normal', 'text-white hover:bg-[#ff6425]/50 dark:hover:bg-dark-600 dark:text-white');
        TallStackUi::personalize('sideBar.item')
        ...
        TallStackUi::personalize('sideBar.item')
            ->block('group.button', 'text-white hover:bg-[#ff6425]/50 dark:hover:bg-dark-600/50 flex w-full items-center gap-x-3 rounded-md p-2 text-left text-sm font-semibold transition-all dark:text-white cursor-pointer');
        TallStackUi::personalize('sideBar.item')
            ->block('group.icon.collapse.rotate', 'text-white rotate-180 dark:text-white')
            ->block('group.icon.collapse.base', 'text-white ml-auto h-4 w-4 shrink-0 transition-all dark:text-white');
